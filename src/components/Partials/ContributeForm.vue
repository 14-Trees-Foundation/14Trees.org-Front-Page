<template>
	<div>
		...
		<div v-show="contribution_type">
 		<Glide :triggerOnMount="false" ref="glideAnimate" :delay="1"><ClientOnly>
				 <form action="#" method="POST" id="pledgeForm" @submit="checkAndSubmitForm" class="mt-18 md:ml-12 md:mb-32 mb-12">
					<div v-if="message" class="md:max-w-1/2-screen mx-auto">
						...
						<div class="h-96 w-full border border-gray-300">
							<div class="text-center mt-48">IMAGE here?</div>
						</div>
					</div>
					<div v-else>
										 ...
										 <div class="px-4 py-3 bg-white dark:bg-dark-grey sm:p-3">
											 <div class="grid grid-cols-6 gap-6">
												 <div class="col-span-6 sm:col-span-3">
													 ...
													 <input type="text" v-model="formData.donor.first_name" name="first_name" required
												 ...
												 <div class="col-span-6 sm:col-span-3">
													 ...
													 <input type="text" v-model="formData.donor.last_name" name="last_name" id="last_name"
														 autocomplete="family-name" required
														 class="input-field" />
												 </div>
												 <div class="col-span-6 sm:col-span-4 relative pt-3">
													 <!-- <label for="email_address"
														 class="block text-sm font-medium text-gray-700">Email
														 Address</label> -->
													<input type="email" v-model="formData.donor.email_id" name="email_address"
														id="email_address" autocomplete="email" required placeholder="Email Address"
														class="input-field peer"/>
													<label for="email_address" class="absolute left-0 -top-3 text-gray-600 
												 ...
												 <div class="col-span-6 sm:col-span-3">
													 ...
													 <input type="tel" v-model.number="formData.donor.phone" name="phone_number"
										 ...
										 <div class="px-4 bg-white dark:bg-dark-grey space-y-6 sm:p-3">
											 <fieldset>
												 <div class="mt-4 space-y-4">
													 <div class="flex items-start">
														 <div class="flex items-center h-5">
															 <input id="updates" name="updates" v-model="formData.donor.interest.updates"
																 type="checkbox" class="input-checkbox" />
														 </div>
														 <div class="ml-3 text-sm">
													 ...
													 <div class="flex items-start">
														 <div class="flex items-center h-5">
															 <input id="newsletter" name="newsletter" v-model="formData.donor.interest.newsletter"
																 type="checkbox" class="input-checkbox" />
														 </div>
														 <div class="ml-3 text-sm">
															 <label for="candidates"
...
export default {
    components: { Modal, OrderSummary, ProgressCheckPoints, ContributionType, Glide },
	props: {
		fromCampaign: ""
	},
	data: function () {
	...
	methods: {
		selectType(type) {
			...
			if (type === "foreign") {
				this.currency = "usd"
				this.message = `
					For overseas transcations, we have partnered with <a class="text-slate-500" href="https://terrepolicycentre.com/"> TERRE Policy Centre</a> to collect payments on our behalf.
					Please go through our Terms and Conditions for more info.
				`
			}
	...
	computed: {
		...
		contributionFilled: function () {
			return typeof this.formData.contribution.trees === 'number' &&
				this.formData.contribution.trees > 0 &&
				this.formData.contribution.campaign?.name?.length > 0
		},
